...
**Document ID:** IDENT-011 | Version 1.3 | Status: Approved (Owner Decision, 2026-09-05); member-management + invite rows annotated as implemented Phase 2.5 (2026-09-10); members-list row annotated as implemented Phase 2.6 (2026-09-10) | Phase: 2A (Architecture Finalization)
...
| `GET /workspaces/:id/members`                  | `... → requirePermission(members.read)`                                                                       | **Implemented Phase 2.6** — active members only, via `listActiveMembershipsForWorkspace()`. See `phase-2-6-implementation-report.md`.                                                                                                                                       |
| `POST /workspaces/:id/members/invite`          | `... → requirePermission(members.invite)`                                                                     | **Implemented Phase 2.5** via Clerk's Organization Invitation API — request body is `{ emailAddress }` only, no role; creates no local `workspace_membership` row (appears only after Clerk-side acceptance + existing sync). See `phase-2-5-implementation-report.md` §14. |
